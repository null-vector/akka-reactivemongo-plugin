...
private object EventAdapterMacroFactory {
  ...
  def mappingOf[E](context: blackbox.Context)(implicit
      eventTypeTag: context.WeakTypeTag[E]
  ): context.Expr[BSONDocumentMapping[E]] = {
    import context.universe._
    val (imports, implicits) = implicitMappingsFor(context)(
      eventTypeTag.tpe,
      noImplicitForMainType = true
    )
    val code                 =
      q"""
...
//    context.echo(context.enclosingPosition, s"${code.toString()}")
  ...
  def mappingOfWithBeforeRead[E](
      context: blackbox.Context
  )(beforeRead: context.Expr[BSONDocument => BSONDocument])(implicit
      eventTypeTag: context.WeakTypeTag[E]
  ): context.Expr[BSONDocumentMapping[E]] = {
    import context.universe._
    val (imports, implicits) = implicitMappingsFor(context)(
  ...
  def adaptWithTags[E](
      context: blackbox.Context
  )(withManifest: context.Expr[String], tags: context.Expr[Set[String]])(implicit
      eventTypeTag: context.WeakTypeTag[E]
  ): context.Expr[EventAdapter[E]] = {
    import context.universe._
    buildAdapterExpression(context)(
      q"new org.nullvector.EventAdapterMapping[${eventTypeTag.tpe}]($withManifest, $tags)"
    )
  }

  def adaptWithPayload2Tags[E](
      context: blackbox.Context
  )(withManifest: context.Expr[String], tags: context.Expr[E => Set[String]])(implicit
      eventTypeTag: context.WeakTypeTag[E]
  ): context.Expr[EventAdapter[E]] = {
    import context.universe._
    buildAdapterExpression(context)(
  ...
  def adapt[E](context: blackbox.Context)(withManifest: context.Expr[String])(implicit
      eventTypeTag: context.WeakTypeTag[E]
  ): context.Expr[EventAdapter[E]] = {
    import context.universe._
    buildAdapterExpression(context)(
      q"new org.nullvector.EventAdapterMapping[${eventTypeTag.tpe}]($withManifest)"
    )
  }

  private def buildAdapterExpression[E](
      context: blackbox.Context
  )(createEventAdapter: context.universe.Tree)(implicit
      eventTypeTag: context.WeakTypeTag[E]
  ): context.Expr[EventAdapter[E]] = {

    import context.universe._
    val eventType           = eventTypeTag.tpe
    val (imports, handlers) =
      implicitMappingsFor(context)(eventType, noImplicitForMainType = false)
    val code                =
      q"""
        import reactivemongo.api.bson._
  ...
  private def implicitMappingsFor(context: blackbox.Context)(
      ...
      noImplicitForMainType: Boolean = false
    ...
    val (mappedTypes, mappingCode) = caseClassTypes.flatMap { aType =>
      val isWriterDefined =
        context.inferImplicitValue(appliedType(bsonWriterType, aType)).nonEmpty
      val isReaderDefined =
        context.inferImplicitValue(appliedType(bsonReaderType, aType)).nonEmpty
  ...
  private def extractCaseTypes(context: blackbox.Context)(
      rootType: context.universe.Type
  ): org.nullvector.Tree[context.universe.Type] = {
    import context.universe._
    ...
    def extractAll(
        caseType: ContextType,
        alreadyExtracted: List[ContextType] = Nil
    ): org.nullvector.Tree[ContextType] = {
      def isSupportedTrait(aTypeClass: ClassSymbol) =
        aTypeClass.isTrait && aTypeClass.isSealed && !aTypeClass.fullName
          .startsWith("scala")

      def isCaseOrTrait(aType: ContextType) =
        aType.typeSymbol.asClass.isCaseClass || isSupportedTrait(
          aType.typeSymbol.asClass
        )
      ...
      if (
        caseType.typeSymbol.asClass.isCaseClass && !alreadyExtracted.contains(
          caseType
        )
      ) {
        Tree(
          ...
          caseType.decls.toList
            ...
            .collect {
              ...
              case aType
                  if aType.typeSymbol.owner.isType &&
                    aType.typeSymbol.owner.asType.toType =:= enumType =>
                List(Tree(aType))
              case aType if isCaseOrTrait(aType) =>
                List(extractAll(aType, caseType :: alreadyExtracted))
              case aType                         =>
                extractCaseClassesFromTypeArgs(aType)
                  .map(arg => extractAll(arg, caseType :: alreadyExtracted))
            }
            .flatten
        )
      } else if (isSupportedTrait(caseType.typeSymbol.asClass)) {
        val writerNotDefined = context
          .inferImplicitValue(appliedType(bsonWriterType, caseType))
          .isEmpty
        val readerNotDefined = context
          .inferImplicitValue(appliedType(bsonReaderType, caseType))
          .isEmpty
        if ((writerNotDefined && readerNotDefined) || caseType =:= rootType)
          Tree(
            caseType,
            caseType.typeSymbol.asClass.knownDirectSubclasses
              .map(aType => extractAll(aType.asClass.toType, caseType :: alreadyExtracted))
              .toList
          )
        else Tree.empty
      } else Tree.empty
