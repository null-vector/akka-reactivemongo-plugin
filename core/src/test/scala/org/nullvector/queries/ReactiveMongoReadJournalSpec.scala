...
class ReactiveMongoReadJournalSpec() extends FlatSpec with TestKitBase with ImplicitSender
  ...
  it should "Infinite Events by tag with Custom RefreshInterval" in {
    ...
    Thread.sleep(1000)
    Await.ready(Source(1 to 3).mapAsync(amountOfCores) {
      idx =>
        val pId = s"${
          prefixReadColl
        }_$idx-${
  ...
  it should "Infinite persistenceId" in {
    ...
    Await.ready(Source(1 to 10).mapAsync(10) {
      collId =>
        reactiveMongoJournalImpl.asyncWriteMessages((1 to 25).map {
          jIdx =>
            val pId = s"${
              prefixReadColl
            }_$collId-${
