...
class ReactiveMongoScalaReadJournalImpl(
    system: ExtendedActorSystem,
    protected val collectionNames: List[String]
) extends akka.persistence.query.scaladsl.ReadJournal
    with EventsQueries
    with PersistenceIdsQueries
    with ReactiveMongoScalaReadJournal {
  ...
  protected implicit lazy val dispatcher: ExecutionContext    =
    system.dispatchers.lookup("akka-persistence-reactivemongo-dispatcher")
  ...
  protected def filterByOffset(offset: Offset): BSONDocument = {
    offset match {
      case ObjectIdOffset(bsonObjectId) =>
        BSONDocument("_id" -> BSONDocument("$gt" -> bsonObjectId))
