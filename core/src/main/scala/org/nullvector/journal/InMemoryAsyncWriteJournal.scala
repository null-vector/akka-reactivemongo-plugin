...
class InMemoryAsyncWriteJournal(val system: ActorSystem) extends AsyncWriteJournalOps {
  ...
  def asyncWriteMessages(
    ...
    Source(messages)
      .mapAsync(1)(atomic =>
        Source
          .future(eventSerializer.serialize(atomic.payload))
          .mapConcat(identity)
          .map(slized => persistentRepr2EventEntry _ tupled slized)
          .runWith(Sink.seq)
          .flatMap(events =>
            persistInMemory
              .addEvents(atomic.persistenceId, events)
              .transform(tried => Try(tried.map(_ => ())))
          )
      )
      .runWith(Sink.seq)
  }
