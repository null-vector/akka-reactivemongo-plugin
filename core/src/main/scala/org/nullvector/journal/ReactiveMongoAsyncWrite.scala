...
trait ReactiveMongoAsyncWrite {
  ...
  def asyncWriteMessages(messages: Seq[AtomicWrite]): Future[Seq[Try[Unit]]] = {
    for {
      ...
      atomicDocs <- Future.traverse(messages) { atomic =>
                      serializer
                        .serialize(atomic.payload)
                        .map(_.map { case (serializedRep, tags) =>
                          rep2doc(serializedRep, tags) -> tags
                        })
                        .map { docs =>
                          BSONDocument(
  ...
  def rep2doc(persistentRepr: PersistentRepr, tags: Set[String]): BSONDocument =
    BSONDocument(
