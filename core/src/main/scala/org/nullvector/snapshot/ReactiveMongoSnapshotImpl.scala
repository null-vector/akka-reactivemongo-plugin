...
class ReactiveMongoSnapshotImpl(
  ...
  def loadAsync(
    ...
    def generateSnapshotFromDocument(maybeDoc: Option[BSONDocument]) =
      maybeDoc match {
        case Some(document) =>
          logger.debug(
            s"Document snapshot for persistenceId:$persistenceId loaded successfully"
          )
          val payloadDoc = document.getAsOpt[BSONDocument](
            Fields.payload
          ) orElse
            document.getAsOpt[BSONDocument](Fields.snapshot_payload) getOrElse
            (throw new Exception(
              s"No payload found for snapshot with persistenceId:$persistenceId"
            ))

          val deserializePayload =
            document.getAsOpt[String](Fields.manifest) match {
              case Some(manifest) =>
        ...
        case None           =>
          logger.debug(
            s"No Document snapshot found from mongo for persistenceId$persistenceId"
          )
    ...
    case Success(Some(snapshot)) =>
      logger.debug(
        s"Deserialization completed for snapshot with persistenceId:$persistenceId and sequenceNr:${snapshot.metadata.sequenceNr}"
      )
    case Success(None)           =>
      logger.debug(
        s"Deserialization completed. No snapshot offer for persistenceId:$persistenceId"
      )
    case Failure(ex)             =>
      logger.error(
        s"Deserialization failed for snapshot with persistenceId:$persistenceId",
        ex
      )
  }
        ...
        case _                  =>
          for {
            ...
            tryed    <- insertDoc(
                          coll,
                          metadata,
                          rep.payload.asInstanceOf[BSONDocument],
                          Option(rep.manifest)
                        )
          } yield tryed
  ...
  private def insertDoc(
    ...
    coll
      ...
      .map(result =>
        if (result.writeErrors.isEmpty) Success(())
        else
          Failure(
            new Exception(result.writeErrors.map(_.toString).mkString("\n"))
          )
      )
  }
